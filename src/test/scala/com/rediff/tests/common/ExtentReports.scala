...
//    if (!className.endsWith("$")) {
//      if(!new java.io.File("target/test-reports/extent-reports/Alltests/Index.html").isFile) {
//        println("creating new exent spark reporter for first time")
//        reporter = new ExtentSparkReporter("target/test-reports/extent-reports/Alltests")
//        reporter.config().setDocumentTitle(className)
//        reporter.config().setReportName(className)
//      }
//      extent = new ExtentReports
//      extent.attachReporter(reporter)
//
